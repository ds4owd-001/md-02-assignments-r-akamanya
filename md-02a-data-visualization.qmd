...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    Link: <https://www.perplexity.ai/search/e3360931-2211-42c0-8c50-9679d836db67?s=u>
...
-   *Non-Numerical Categorical, such as 'names of continents',* *can also be ordinal.*

-   *Numerical (continuous) such as 'life Expectancy'*

-   *Numerical (discrete) like whole numbers*
...
ggplot(gapminder_2007, aes(x = continent, 
  ...
  labs(title = "Life Expectancy across Five Continents in 2007",
       subtitle = "Africa will lead",
       x = "Continent",
...
```{r}
#| eval: true
ggplot(gapminder_summary_continent_year, aes(x = year, 
                                             y = lifeExp,
                                             color = continent))+
  geom_line()+
  geom_point()+
   labs(title = "Timeseries Plot for Life Expectancy",
       subtitle = "(1952 - 2007)",
       x = "Year",
       y = "Life Expectancy in Years",
       color = "Continent")
...
```{r}
#| eval: true
ggplot(gapminder_summary_2007, aes(x = continent, 
                           y = count))+
  geom_col()+
  labs(title = "Observations across Five Continents in 2007",
       subtitle = "-----",
       x = "Continent",
       y = "Count"
       )
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: *The geom_bar function counts the observations in the gapminder_2007 dataset, whereas the geom_col function has to be fed with summary stats of the observation counts from the created data frame: gapminder_summary_2007*
...
*Both functions compute and return frequencies (our counts) of the specified variable.*

5.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:

*The seq() function helps define bin sizes and the min and maximum numbers on the x-axis. It is helpful to query your data in a given variable to identify the max and min (extents) inorder to create a representative plot.*
...
ggplot(gapminder_2007, aes(x = lifeExp, 
  ...
  geom_histogram(col = "grey30",
                 breaks = seq(40,85,2.5))+
  labs(title = "Frequencies of life Expectancies as observed in Continents in 2007",
       subtitle = "-----",
       x = "Life Expectancy",
       y = "Frequency",
       fill = "Continent"
...
ggplot(gapminder_2007, aes(x = gdpPercap, 
  ...
  labs(title = "Scatter of life Expectancies versus GDP per capita across Continents in 2007",
       subtitle = "-----",
...
*The object stores Color codes and country names as assigned in the Gapminder dataset*
